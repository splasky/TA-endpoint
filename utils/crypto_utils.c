                ...
                unsigned char *ciphertext, int ciphertext_len) 
{
  ...
  for(int i = 0; i < new_len; i += AES_BLOCK_SIZE){
    ...
    if ((status = mbedtls_aes_crypt_cbc(
                ...
                unsigned char *plaintext, int plaintext_len) 
{
  ...
  /* set decryption key */
  if ((status = mbedtls_aes_setkey_dec(&ctx, key, keybits))
        != EXIT_SUCCESS) {
    err = "set aes key failed";
    goto exit;
  }
   ...
   */
   ...
   for(int i = 0;i < ciphertext_len; i += AES_BLOCK_SIZE){
    ...
    if ((status = mbedtls_aes_crypt_cbc(&ctx,  MBEDTLS_AES_DECRYPT ,
                                      AES_BLOCK_SIZE, iv, buf, buf)) != 0) {
      err = "aes decrpyt failed";
      goto exit;
    }
